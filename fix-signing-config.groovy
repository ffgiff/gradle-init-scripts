...
final boolean oneConnectedDevice = 0 ==
        new ProcessBuilder(project.android.adbExe.getPath(), 'get-serialno')
                .start()
                .waitFor()
String signCert =
...
if (oneConnectedDevice || null != System.getenv('ANDROID_SERIAL')) {
    final Process getName =
        new ProcessBuilder(project.android.adbExe.getPath(), 'shell', 'getprop', 'ro.product.name')
                .redirectInput(ProcessBuilder.Redirect.INHERIT)
                .redirectError(ProcessBuilder.Redirect.INHERIT)
                .start()
    getName.waitFor()
    final Process getType =
        new ProcessBuilder(project.android.adbExe.getPath(), 'shell', 'getprop', 'ro.build.type')
                .redirectInput(ProcessBuilder.Redirect.INHERIT)
                .redirectError(ProcessBuilder.Redirect.INHERIT)
                .start()
    getType.waitFor()
...
//            new BufferedReader(new InputStreamReader(getType.getInputStream(), 'US-ASCII'))) {
    try {
        final BufferedReader nameReader = new BufferedReader(
                new InputStreamReader(getName.getInputStream(), 'US-ASCII'))
        final BufferedReader typeReader = new BufferedReader(
                new InputStreamReader(getType.getInputStream(), 'US-ASCII'))
        signFor = nameReader.readLine() + '_' + typeReader.readLine()
        nameReader.close()
        typeReader.close()
    } catch (final IOException e) {
        project.logger.log(0, e.getLocalizedMessage(), e)
    }
}
...
project.android {
    signingConfigs {
        signingConfig {
            storeFile new File(buildscript.getSourceFile().getParentFile(),
                    "${signCert}.keystore")
